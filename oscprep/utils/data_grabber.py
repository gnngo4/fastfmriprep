...
class bids_reader:
    ...
    def get_session_list(self, subj_id):
        ...
        if subj_id[:4] != 'sub-': subj_id = f"sub-{subj_id}"
    ...
    def get_bold_list(self, subj_id, sess_id, ignore_tasks = [], specific_task = None, full_path_flag=False, suffix='bold.nii.gz'):
        ...
        if subj_id[:4] != 'sub-': subj_id = f"sub-{subj_id}"
        if sess_id[:4] != 'ses-': sess_id = f"ses-{sess_id}"
    ...
    def get_t1w_list(self, subj_id):
        ...
        assert len(t1w_list) != 0, f"No T1w acquisitions were found."
