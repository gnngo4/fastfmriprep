...
def init_brainmask_mp2rage_wf(name="skullstrip_mp2rage_wf"):
    """
    ...
    synthstrip_native = pe.Node(
        SynthStrip(out_file="brain.nii.gz", mask_file="mask.nii.gz"),
        name="synthstrip_native",
    )

    # Upsample MP2RAGE
    upsample = pe.Node(
        fsl.FLIRT(output_type="NIFTI_GZ", out_file="upsample.nii.gz"),
    ...
    apply_mask = pe.Node(
        fsl.ApplyMask(out_file="brain.nii.gz"),
        name="apply_mask",
    )
    ...
    workflow.connect(
        [
            ...
            (
                inputnode,
                upsample,
                [("upsample_resolution", "apply_isoxfm")],
            ),
            ...
            (
                resample_up_mask,
                combine_masks,
                [(("out_file", _listify), "operand_files")],
            ),
...
def init_brainmask_mprage_wf(name="skullstrip_mprage_wf"):
    """
