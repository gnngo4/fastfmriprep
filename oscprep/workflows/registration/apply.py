...
def init_apply_fmap_to_bold_wf(
    ...
    outputnode = pe.Node(
        niu.IdentityInterface(fields=["target_ref", "fmap_bold"]),
        name="outputnode",
    )
    ...
    if use_fsl_gre_fmap:
        from oscprep.interfaces.fsl_prepare_fieldmap import (
            FSLPrepareFieldmap,
        )
        ...
        workflow.connect(
            [
                ...
                (
                    mask_fmap_mag,
                    prepare_fmap,
                    [("out_file", "magnitude_image")],
                ),
                (
                    fmap2epi,
                    prepare_fmap,
                    [("output_image", "phase_image")],
                ),
                (
                    inputnode,
                    prepare_fmap,
                    [(("fmap_metadata", _get_delta_te), "deltaTE")],
                ),
                (
                    prepare_fmap,
                    outputnode,
        ...
        # Mask fmap
        mask_fmap = pe.Node(fsl.ApplyMask(), name="mask_fmap")
        # Convert fmap units from Hz to rads
        convert_to_rad = pe.Node(
            fsl.BinaryMaths(operation="mul", operand_value=6.28),
            name="convert_to_rad",
        )
...
def init_apply_bold_to_anat_wf(slab_bold_quick=False, name="apply_bold_to_t1_wf"):
    ...
    from oscprep.interfaces.bold_to_anat_transform import (
        BoldToT1Transform,
    )
    ...
    workflow.connect(
        [
            ...
            (
                apply_bold_to_t1,
                outputnode,
                [("t1_bold_path", "t1_space_bold")],
            ),
            (
                inputnode,
                apply_bold_ref_to_t1,
                [
                    ("bold_ref", "in_file"),
