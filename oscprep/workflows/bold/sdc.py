...
def init_bold_sdc_wf(
    ...
    apply_fmap_to_bold_wf = init_apply_fmap_to_bold_wf(
        use_fsl_gre_fmap=use_fsl_gre_fmap, fmap_metadata=fmap_metadata
    )
    ...
    vsm_to_warp = pe.Node(
        fsl.ConvertWarp(abswarp=True, output_type="NIFTI_GZ"),
        name="vsm_to_warp",
    )

    unwarp_bold = pe.Node(fsl.ApplyWarp(), name="unwarp_bold")
    ...
    workflow.connect(
        [
            ...
            (
                apply_fmap_to_bold_wf,
                get_vsm,
                [("outputnode.fmap_bold", "fmap_in_file")],
            ),
            (
                inputnode,
                get_vsm,
            ...
            (
                ...
                [
                    ...
                    (
                        ("bold_metadata", _get_fsl_shift_direction),
                        "shift_direction",
                    ),
                ],
            ),
...
def _get_fsl_shift_direction(metadata_dict):
    assert (
        "PhaseEncodingDirection" in metadata_dict
    ), "PhaseEncodingDirection not found in metadata."
