...
class FSLPrepareFieldmapInputSpec(CommandLineInputSpec):
    phase_image = File(
        ...
        mandatory=True,
        position=0,
        desc="phasediff image",
    )
    magnitude_image = File(
        argstr="%s",
        exists=True,
        mandatory=True,
        position=1,
        desc="skullstripped magnitude image",
    )
    out_image = File(
        argstr="%s",
        position=2,
        desc="skullstripped fieldmap image (rad/s)",
    )
    deltaTE = traits.Float(
        argstr="%s",
        mandatory=True,
        position=3,
        desc="echo time difference (ms)",
    )
